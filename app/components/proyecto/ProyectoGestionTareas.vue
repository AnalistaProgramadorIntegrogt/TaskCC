            ...
            <div>
              ...
              <div class="bg-base-200/60 rounded-xl p-3 mb-4 flex flex-wrap gap-3 text-xs">
                <div>
                  ...
                  <span class="ml-1 font-bold text-primary capitalize">{{ grupo.recurrencia_tipo || 'Manual' }}</span>
                </div>
        ...
        <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          >
            ...
            <div>
              ...
              <!-- Recurrencia -->
              <div class="bg-base-200/60 rounded-xl p-2.5 mb-3 flex flex-wrap gap-3 text-xs">
                <div>
                  <span class="font-semibold text-base-content/60">Recurrencia:</span>
                  <span class="ml-1 font-bold text-accent capitalize">{{ grupo.recurrencia_tipo || 'Manual' }}</span>
                </div>
                <div v-if="grupo.recurrencia_tipo === 'diario'">
                  <span class="font-semibold text-base-content/60">Día asignado:</span>
                  <span class="ml-1 font-bold text-secondary">{{ getDiaSemanaNombre(grupo.dia_semana) }}</span>
...
const copiarGrupoAlProyecto = async (grupoDefault: any) => {
  ...
  try {
    ...
    const { data: newG, error: gErr } = await supabase.from('grupos').insert({
      ...
      es_predeterminado: false,
      proyecto_id: props.proyectoId,
      recurrencia_tipo: grupoDefault.recurrencia_tipo,
      dia_semana: grupoDefault.dia_semana,
      dia_mes: grupoDefault.dia_mes,
      intervalo_meses: grupoDefault.intervalo_meses || 1
    }).select().single()
