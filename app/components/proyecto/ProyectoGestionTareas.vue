              ...
              <div class="flex justify-between items-start mb-3 gap-2">
                ...
                <div class="flex items-center gap-1">
                  <button 
                    ...
                    title="Imprimir códigos QR de todas las tareas del grupo"
                  >
                    <QrCode :size="14" />
...
const guardarTarea = async () => {
  ...
  // Validar si ya existe una tarea con el mismo nombre (insensible a mayúsculas/minúsculas)
  const existe = catalogTareas.value.some(
    (t: any) => t.nombre && t.nombre.trim().toLowerCase() === nombreLimpio.toLowerCase()
  )
  if (existe) {
    showToast(`Ya existe una tarea registrada con el nombre "${nombreLimpio}".`, 'error')
    return
  }
